...
def convert_ixic_data_to_csv(json_file, csv_file):
    """
    将IXIC (纳斯达克指数) JSON数据转换为CSV格式
    """
    ...
    print(f"成功转换 {len(df)} 条IXIC记录到 {csv_file}")
    print("\n前5行数据预览：")
    print(df.head())
...
if __name__ == "__main__":
    # 转换IXIC数据
    df = convert_ixic_data_to_csv('ixic.json', 'nasdaq_index.csv')